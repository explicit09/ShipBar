...
- `ProjectWorkspaceView` now receives an explicit `backToProjects` closure and exposes a labeled leading Back to Projects button. Every root integration clears `selectedProjectID`.
...
- The canonical QA HTML links retained compact/wide screenshots, representative AX text, and a command/exit summary.
